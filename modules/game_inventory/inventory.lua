InventorySlotStyles = {
  ...
  function init()
    ...
    inventoryWindow = g_ui.loadUI('inventory', modules.game_interface.getTopRightPanel())
    --inventoryWindow:disableResize()
    --inventoryWindow:getChildById('minimizeButton'):hide()
    --inventoryWindow:getChildById('closeButton'):hide()
    ...
    walkModeButton = inventoryWindow:recursiveGetChildById('walkModeBox')
    ...
    walkModeRadioGroup = UIRadioGroup.create()
    walkModeRadioGroup:addWidget(walkModeButton)
    walkModeRadioGroup:addWidget(chaseModeButton)
  
    connect(fightModeRadioGroup, { onSelectionChange = onSetFightMode })
    connect(pvpModeRadioGroup, { onSelectionChange = onSetPVPMode })
    connect(walkModeRadioGroup, { onSelectionChange = onSetChaseMode })
  ...
  function update()
    ...
    if chaseMode == DontChase then
      walkModeRadioGroup:selectWidget(walkModeButton)
    elseif chaseMode == ChaseOpponent then
      walkModeRadioGroup:selectWidget(chaseModeButton)
    end
  ...
  function onSetChaseMode(self, selectedWalkButton)
    if selectedWalkButton == nil then return end
    local buttonId = selectedWalkButton:getId()
    local chaseMode
    if buttonId == 'chaseModeBox' then
      chaseMode = ChaseOpponent
    elseif buttonId == 'walkModeBox' then
      chaseMode = DontChase
    end
  ...
  function onFreeCapacityChange(player, freeCapacity)
    if freeCapacity >= 1000000 then
      --capLabel:setText(tr('Cap') .. ': 0')
      capLabel:setText('0')
    else
